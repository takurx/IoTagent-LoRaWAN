...
//const LPP_TEMPERATURE_NAME = 'TempC_SHT';
...
function decodePayload(payload) {
	let decodedObject = {};
	//paylod = 0x03, 0x67, 0x02, 0x55;
	config.getLogger().info(context, 'Decoding CaynneLPP message:' + payload);
	try {
		decodedObject = decodeCayenneLpp(payload);
	} catch (e) {
		config.getLogger().error(context, 'Error decoding CaynneLPP message:' + e);
		return;
	}
...
function decodeCayenneLpp(bufferBase64) {
	...
	if (buffer && validateCayenneLppSize(buffer)) {
		...
		while (cursor < buffer.length) {
			const channel = buffer.readUInt8(cursor);
				...
				switch (type) {
					...
					case LPP_ACCELEROMETER:
						if (cursor + LPP_ACCELEROMETER_SIZE > buffer.length) {
							...
							const varAccX = buffer.readInt16BE(cursor) / 1000.0;
							cursor += 2;
							const varAccY = buffer.readInt16BE(cursor) / 1000.0;
							cursor += 2;
							const varAccZ = buffer.readInt16BE(cursor) / 1000.0;
							cursor += 2;
					...
					case LPP_GYROMETER:
						if (cursor + LPP_GYROMETER_SIZE > buffer.length) {
							...
							const varX = buffer.readInt16BE(cursor) / 100.0;
							cursor += 2;
							const varY = buffer.readInt16BE(cursor) / 100.0;
							cursor += 2;
							const varZ = buffer.readInt16BE(cursor) / 100.0;
							cursor += 2;
					...
					case LPP_GPS:
						if (cursor + LPP_GPS_SIZE > buffer.length) {
							...
							const latitude = readInt24BE(buffer, cursor) / 10000.0;
							cursor += 3;
							const longitude = readInt24BE(buffer, cursor) / 10000.0;
							cursor += 3;
							const altitude = readInt24BE(buffer, cursor) / 100.0;
							cursor += 3;
